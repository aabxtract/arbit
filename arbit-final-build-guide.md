# Arbit â€” Final Build Guide
**Programmable Hookathon | Deadline: Sep 10, 2026 | Prize: $10,000**

> **Status (Sep 8): 60 pass + 4 fork opt-in; review fixes landed (audit rows 20-24); preflight needs_evidence, zero hard-blocks; NEXT: submit approval (user decision pending).**
>
> **Budget: $5 MAX plan (measured Sep 7, gas ~0.24 gwei, ETH ~$2470). Badge leaves launch graph (deploys later ~$0.40; setBadge/setMinter are post-launch owner calls). 4-target pack init ~24.5KB bytes ~= 5.1M gas ~= $3.03 + init/LP/buy/Router ~= $0.75-1.10, so gas ~= $3.80-4.15. Value: first buy $1.05 floor + minimal LP ~$0.30. TOTAL ~= $5.15-5.50 at current gas: fits ONLY if gas <= 0.20 gwei at fire time. Watch: cast gas-price on mainnet RPC (div 1e9).**

---

## What We're Building

Arbit is a Uniswap v4 hook where every swap directly moves assets based on who you are. The hook IS the asset engine â€” it collects fees, splits them by participant type, and automatically buys back Arbit token when conditions are met. Every interaction makes the token stronger.

**Three participant flows:**

| Participant | Fee | What happens to assets |
|---|---|---|
| Human trader | 0.30% | 80% â†’ LPs, 20% â†’ buyback pool |
| Registered agent (high rep) | 0.05% | Portion returned as Arbit token reward |
| Unregistered bot | 1.00% | 60% â†’ victim compensation, 40% â†’ buyback + burn |

**Buyback Guard:** Auto-buyback only fires when fee pool â‰¥ `MIN_BUYBACK_AMOUNT` (ARBT units the hook actually holds â€” retune per deployment) AND `BUYBACK_COOLDOWN` has elapsed. Prevents spam triggering and flash loan manipulation. TWAP pricing is explicitly post-hackathon â€” MVP has no oracle dependency by design.

**Stock-pool edge:** pools flagged via `setStockPool` (owner-only) get 2/3 fees (30â†’20, 15â†’10, 5â†’3, 100â†’66) AND 1.5x accrual on every flow. Net effect is a real subsidy: cheaper trades that fill the (shared) burn schedule faster. Demo: standard pool vs AAPL/USDG-style pool side by side.

**The demo story:**
> Human trader swaps â†’ 20% of fee flows to buyback pool â†’ Unregistered bot swaps â†’ 1% tax collected â†’ threshold hit + cooldown elapsed â†’ hook executes buyback automatically â†’ Arbit token supply decreases â†’ token appreciates
...
```
Phase 1 â€” Robinhood Chain Testnet (chain ID 46630)
  â†’ Write + test all contracts
  â†’ Deploy via Foundry to Robinhood Testnet (faucet: faucet.testnet.chain.robinhood.com)
  â†’ Verify all three fee flows work correctly
  â†’ Verify Buyback Guard fires only when conditions met
  â†’ Verify buyback doesn't fire during flash loan attack

Phase 2 â€” Robinhood Chain Mainnet (Programmable.market)
  â†’ Pack via programmable-launch CLI
  â†’ Validate remotely
  â†’ Submit + sign
  â†’ Verify on robinhoodchain.blockscout.com
```
...
| Hook framework | Uniswap v4 `IHooks` implemented directly (pinned `v4-periphery` has no `BaseHook` â€” `src/base/` contains routers only; `src/hooks/` contains `permissionedPools/` only) |
| Token standard | ERC-20 (`src/ArbitToken.sol` â€” fixed 1B supply, packed as the graph's `token` target; NOT a separate UI launch) |
...
| Mainnet deploy | `programmable-launch` CLI â†’ Robinhood Chain (chain ID 4663) |
...
Based on real exploits â€” read before writing code:
...
| Checks-Effects-Interactions everywhere | Reentrancy prevention â€” state before transfers |
| Pool allowlist â€” `PoolId` via `key.toId()`, single source in registry | Cross-pool contamination; anyone can init a pool with your hook |
...
| Fee caps â€” MAX_FEE_BPS hard limit, validated after mask | Griefing prevention |
| `setHook()` onlyOwner + set-once guard | Registry hijack prevention |
| No loops in `beforeSwap` or `afterSwap` | Must be O(1) â€” gas griefing prevention (<50k / <30k targets) |
...
| Trader identity = `hookData`-decoded user, fallback `sender` | `sender` is the router/unlocker (`Hooks.sol:256`), NOT the EOA â€” shared routers collapse all users to one address |
| Buyback Guard â€” cooldown + minimum threshold AND hook token balance | Flash loan + spam prevention; accounting must be backed by real balances |
...
| Buyback uses TWAP â€” POST-HACKATHON (not in MVP) | MVP Guard (cooldown + threshold) only; do NOT claim TWAP until implemented |
...
ArbitToken (ERC-20)
  ...
  - Deployed BY the Router as the graph's token target â€” never a separate launch
  - Hook holds tokens for rewards + victim payouts + burn

ArbitRegistry
  - Agent registration + stake
  - Reputation scores (0â€“1000)
...
ArbitInitializer (graph target, one-shot)
  - Atomic: funds hook â†’ setHook + allowPool (owner EOA can't act in-launch)
    â†’ init pool â†’ seed PERMANENTLY LOCKED position â†’ first buy to wallet
  - Leftover ARBT dust â†’ hook buyback pot; ETH dust â†’ wallet; exact accounting
```

---

## Contract 1 â€” `ArbitRegistry.sol`
...
contract ArbitRegistry is Ownable, ReentrancyGuard {
    ...
    struct Agent {
        ...
        uint256 reputationScore; // 0â€“1000, starts 500
        bool active;
    }
    ...
    function register(uint256 stakeAmount) external nonReentrant {
        ...
        // EFFECT first (CEI) â€” state before any external interaction.
        ...
        // INTERACTION last. NOTE: standard ERC-20 only â€” fee-on-transfer /
    ...
    // View-only fee preview â€” safe to call anywhere, changes nothing.
    function previewFee(
        address swapper,
        uint256 gasPrice
    ...
    // Called by hook ONCE per swap (in beforeSwap) â€” classifies + records pattern.
    ...
    function classify(
        ...
        // Bot detection â€” pattern analysis O(1)
        BotPattern storage pattern = botPatterns[swapper];
...
// Implements IHooks directly â€” pinned v4-periphery has no BaseHook.
...
contract ArbitHook is IHooks, ReentrancyGuard, Ownable {
    ...
    // â”€â”€ Buyback Guard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Prevents spam triggering and flash loan manipulation
    // Flash loans repay in same tx â€” cooldown makes them useless here
    ...
    uint256 public constant MIN_BUYBACK_AMOUNT = 0.05e18; // ARBT units held by hook â€” retune per deployment
    ...
    // Pending rewards for agents â€” claimed separately
    ...
    // â”€â”€ Trader identity â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // `sender` is whoever called PoolManager.swap â€” normally the shared router /
    ...
    // â”€â”€ beforeSwap â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Classifies ONCE + returns dynamic fee override to PoolManager.
    // O(1) â€” no loops, no external calls beyond registry.classify.
    ...
    function beforeSwap(
        ...
        // Verify pool is authorized â€” single source is the registry (PoolId typed).
        ...
        // Single classify per swap â€” result cached for afterSwap.
        ...
        // bps â†’ v4 fee units (hundredths of a bip) + override flag.
    ...
    // â”€â”€ afterSwap â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Reuses the beforeSwap classification (no second classify â€” see above).
    ...
    function afterSwap(
        ...
        // EFFECT â€” clear callback state first (isolate callback state per ToB).
        ...
        // â”€â”€ Asset flows based on participant type â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        if (pType == ArbitRegistry.ParticipantType.HUMAN) {
            ...
            // EFFECT â€” update state before any external interaction
            ...
            // Agent reward â€” portion of fee returned as Arbit tokens
            uint256 rewardAmount = _calculateAgentReward(feeAmount, pType);

            // EFFECT â€” queue reward for agent (claimed separately â€” avoid reentrancy)
            ...
            // Bot â€” 60% victim pool, 40% buyback + burn
        ...
        // â”€â”€ Buyback Guard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        // Only fires when BOTH conditions met AND the hook actually holds ARBT
        // to burn â€” prevents flash loan + spam attacks + empty-burn events.
        // Flash loans must repay in same tx â€” cooldown makes them economically useless.
        // NOTE: threshold is denominated in ARBT units of the hook balance, NOT
        // ether â€” set MIN_BUYBACK_AMOUNT per deployment after funding the hook.
        if (
    ...
    // â”€â”€ Buyback execution â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Burns ARBT the hook actually holds (transfer to dead address), capped by
    // balance. CEI strictly followed. No PoolManager swap in MVP â€” no oracle /
    ...
    function _executeBuyback() internal {
        ...
        // EFFECT â€” zero out pool before any external call
        buybackPool = amount == buybackPool ? 0 : buybackPool - amount;
        lastBuybackTime = block.timestamp;

        // INTERACTION â€” burn by sending to dead address
        IERC20(arbitToken).transfer(address(0xdead), amount);

        emit BuybackExecuted(amount, block.timestamp);
    }

    // â”€â”€ Agent reward claim â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Agents pull their rewards â€” push pattern avoided (reentrancy risk)
    function claimReward() external nonReentrant {
        uint256 reward = pendingRewards[msg.sender];
        require(reward > 0, "No pending reward");

        // EFFECT â€” zero before transfer
        pendingRewards[msg.sender] = 0;

        // INTERACTION â€” transfer Arbit token to agent
        IERC20(arbitToken).transfer(msg.sender, reward);
    }

    // â”€â”€ Victim compensation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    function compensateVictim(address victim, uint256 amount) external onlyOwner nonReentrant {
        ...
        // EFFECT â€” deduct before transfer
        victimPool -= amount;

        // INTERACTION â€” transfer to victim
    ...
    // Pool allowlist lives ONLY in the registry â€” no duplicate mapping here.
...
Real tests live in `test/` (21 green as of Sep 6) â€” the outline below maps 1:1:

```solidity
// test/Registry.t.sol â€” register (effects-first), previewFee tiers
// (human/med/high/low), classify bot (high-freq + gas-spike), reward cap
// 1000, slash floor 0 + deactivate, setHook-once, fuzz fee â‰¤ MAX_FEE_BPS
// test/Hook.t.sol â€” human 20%â†’buyback, agent-HIGH reward + claimReward,
...
// test/Executor.t.sol â€” executor registers itself, swaps as agent identity
// test/Invariant.t.sol â€” pools never negative, queued rewards + victim
// earmark â‰¤ hook ARBT balance, fuzz previewFee cap
```
...
## Testnet Deployment (Phase 1 â€” Robinhood Chain Testnet)

```bash
# Setup (repo already has foundry.toml + lib/ â€” no forge init needed)
...
# Never fallback â€” throw explicitly in scripts
# uint256 deployerKey = vm.envUint("PRIVATE_KEY");

# Deploy registry + mined hook (see script/DeployArbit.s.sol â€” verified on anvil Sep 6)
...
# so deploy a fresh PoolManager on testnet first for functional testing â€”
...
# Static analysis â€” via WSL (Slither needs Linux; see Audit notes). 0 high/medium as of Sep 7.
```

---

## Audit notes (self-audit Sep 7 â€” external audit infeasible before Sep 10 deadline)
...
| # | Finding | Disposition |
|---|---|---|
| 1 | Fee measured on `amount0` regardless of direction | FIXED â€” `_extractFeeAmount` now takes `params.zeroForOne` (`src/ArbitHook.sol`); covered by `test_ReverseDirectionFeeAccounting` |
| 2 | `reentrancy-benign`: pending-cache writes after `registry.classify` external call | Accepted â€” registry is our own `onlyHook`-gated contract; hook is `nonReentrant`; CEI holds for untrusted calls |
| 3 | `timestamp` (buyback cooldown) | Accepted â€” cooldown gating, not randomness; 1h manipulability is economically irrelevant |
| 4 | `tx.gasprice` bot signal | WEAK ON L2 â€” Robinhood L2 gas price is near-constant, so the gas-spike arm rarely fires; high-frequency arm is the real detector. First 3 swaps in a window are always HUMAN (pattern warm-up) â€” one-shot sandwich bots are NOT caught. Documented limitation, not fixable without oracles |
| 5 | `buybackPool`/`victimPool` accrue in pool-input-token units but burn/payouts are ARBT 1:1 | Accepted demo simplification â€” directionally correct (fees â†’ supply reduction), numerically approximate. A production fix needs a price oracle (TWAP â€” post-hackathon) |
| 6 | `compensateVictim` is `onlyOwner`, manual, arbitrary recipient | Accepted trust assumption for hackathon â€” owner can drain victimPool to anyone. Production: victim proofs (e.g. sandwich-attribution claims). Disclose to judges |
| 7 | Heavy `claimReward` payouts can starve later `compensateVictim` (balance < victimPool reverts, no loss) | Accepted â€” payouts never over-spend (revert, not loss); operational rule: compensate victims before large reward epochs |
| 8 | Executor `address(this).call` + assembly, `rescue` ETH `call` | Accepted â€” plain call (not delegatecall), `onlyAgent`-gated; executor is NOT part of the launch graph |
| 9 | Naming/event-indexing infos | Accepted â€” cosmetic |
| 10 | Admission `CROSS_CHAIN_REPLAY` | FIXED â€” immutable `chainId` + deploy-time `block.chainid` check on hook + initializer (mirrors kernel); canonical-PM runtime binding at pack is the real protection (`test_WrongChainDeployReverts`) |
| 11 | Admission `POOL_MANAGER_AUTHENTICATION_MISSING` | VERIFIED â€” `onlyPoolManager` on all 10 callbacks incl. 8 stubs (`test_DirectCallbackBypassReverts`, Cork class) |
| 12 | Admission `UNBALANCED_SETTLEMENT` | VERIFIED by construction â€” hook returns zero deltas and never settles/takes; initializer settles every leg it opens (launch-sim asserts balances) |
| 13 | Stock-pool subsidy (2/3 collection, 1.5x accrual from BASE fee) | ACCEPTED design â€” accruals exceed collections on stock pools by construction (2.25x drain rate per unit volume); solvency still capped by real balance; owner flag is needs-evidence (mutable fee surface). `test/Stock.t.sol` pins exact ratios |
| 14 | `previewFee` shows pre-discount tiers | Accepted â€” stock discount applies in-hook only; preview is the base schedule |
| 15 | `missing-zero-check` on registry `_initializer` | Accepted â€” zero disables in-launch wiring (standalone/testnet mode); documented |
| 16 | `arbitrary-send-erc20` on initializer pull | Accepted â€” source is the immutable launch wallet bound at construction, not caller-supplied |
...
| 18 | Distributor keeper-gating + minOut sandwich surface | ACCEPTED â€” keeper runs buys off fresh quotes (owner-managed key); permissionless TWAP execution is post-hackathon. `test/Distributor.t.sol` (5 tests) + fork money-loop proof |
| 19 | Registry keeper role (reward/slash without hook) | ACCEPTED centralization for module path â€” owner-managed, disclosed; hook path unchanged |
| 20 | REVIEW (Sep 8, external): poke-gigagas overflow bricks pricing (DoS) | FIXED critical â€” `_isBot` uses division (`gasPrice/3 > avg`), overflow impossible; `test_Review_PokeCannotBrickPricing` |
| 21 | REVIEW: multi-agent reward overcommit | FIXED â€” global `totalPromised` cap vs backing; claims release commitments; `test_Review_MultiAgentBackingHolds` |
| 22 | REVIEW: burn consumes promised rewards | FIXED â€” burns reserve `victimPool + totalPromised` first; `test_Review_BurnPreservesPromises` |
| 23 | REVIEW: distributor victim over-accounting | FIXED â€” 80/20 split applies to claimed amounts only, invariant `victimPool â‰¤ balance` holds inductively; `test_Review_VictimReserveStaysSolvent` |
| 24 | REVIEW: hookData attribution for others | BY DESIGN (standard v4 router pattern) â€” rewards no-op for inactive agents; pinned in test, no code change |

NOT covered by this audit (state explicitly): economic soundness of fee levels, oracle/TWAP design (doesn't
exist yet), the Programmable graph/pack layer, offchain demo scripts, upgrade story (none â€” immutable by design).
If judges ask: "self-audited + Slither-clean of high/medium; no external audit before deadline" â€” maturity, not weakness.

---

## Mainnet Deploy (Phase 2 â€” Programmable Custom Launch V4, verified Sep 7)
...
`validate --remote --network robinhood` / `submit --network robinhood` commands DO NOT EXIST â€” do not use them.

**Step 0 â€” confirm the lane is open (no key needed, re-check on deploy day):**
```bash
...
**Step 1 â€” install the pinned CLI (checksum-verified, never an npm package of the same name):**
```bash
...
(cd /tmp && sha256sum -c plc.tgz.sha256)   # must print OK â€” expected 9d7d26a7â€¦
npm install --global /tmp/plc.tgz
programmable-launch --version              # must print 4.1.0
```

**Step 2 â€” API key (you do this, never the agent):**
Create at programmable.market/developers/api-keys with `custom-launch:create` + chain `4663` grant.
Export `PROGRAMMABLE_API_KEY` (env or OS secret store â€” the CLI has no `--api-key` flag).

**Step 3 â€” token + metadata (code: `src/ArbitToken.sol`, fixed 1B ARBT, no mint/tax/pause):**
The token is NOT launched separately via UI â€” it is the `componentKind: "token"` target inside the
packed graph, with name/symbol/description/image/links in `projectMetadata`. Before packing you must have,
and never invent: token name + symbol, â‰¥20-byte description, real PNG (or single-frame GIF) + its public
...
**Step 4 â€” pack config (`programmable-launch.config.json`, schema `programmable.launch-pack-config.v4`):**
Targets (3â€“16): `arbit-token` (token), `arbit-registry` (other, ctor = token locator), `arbit-hook`
(hook, ctor = PoolManager `0x8366a39CC670B4001A1121B8F6A443A643e40951` + registry locator + token locator,
`declaredHookPermissions: ["beforeSwap","afterSwap"]`, salt = `deterministic-hook-permission-grind-v1` â€”
pack grinds the CREATE2 salt itself, no HookMiner needed). Pool: token+hook IDs, `fee: 8388608`
(dynamic â€” structurally supported), `tickSpacing`, native-ETH quote. Plus `fundingPlan` (fund-and-launch:
`initialLiquidityWei` + `initialBuyWei` â‰¥ server quote â‰ˆ$1 + `maxLaunchValueWei` + `maxGasCostWei`),
`liquidityModel` (`external-concentrated-liquidity` = we seed our own position â€” pool init alone is NOT
liquid), `behaviorScenarioInputs` (our seed/swap/claim demo steps), `agentAttestation`.

**Step 5 â€” the state machine (CLI never signs or broadcasts):**
```bash
...
# ROBINHOOD_* findings (see coverage verdict below) â€” resolve with devrel FIRST.
...
**Step 6 â€” post-finality wiring (same tx or immediately after, order matters):**
...
**âš ï¸ Coverage verdict (live `/v4/chains/4663/launch-coverage`, Sep 7) â€” read before packing:**
...
custom victim/buyback settlement, tickSpacing 10) is OUTSIDE it â†’ preflight may return
...
**BUT â€” precedent found Sep 7 (changes the calculus): "Robinhood Clean Room" (RHCR), finalized
Sep 3 on mainnet under profile 4.0.0, is a CUSTOM bonding-curve hook (hook `0xa3b48907â€¦`, token
`0x15fca4â€¦`, vault `0x0cddef91â€¦`, all `exact_match`): custom pricing via return deltas, 5% native fee
decaying to zero over 24h, virtual reserves, custom settlement vault â€” strictly WILDER than our design
...
SELFDESTRUCT, missing/invalid callback auth, noncanonical PoolManager, missing callback impl) â€” our
...
stamped PoolKey" (20bps to `0xD885â€¦079da`). The docs-blessed composition point is the kernel's
view-only module slot (capped fee override, zero custom deltas â€” sized exactly for our fee tiers, with
bot-pattern writes moved to off-swap pokes). So the single open question is kernel-vs-module-vs-custom
under 4.1 â€” see devrel draft below. "Shard" was NOT found in the V4 finalized feed (only RHCR) or
explore â€” link it if you meant a specific project and I'll tear it down the same way.

**V4 admission hard-blocks relevant to us (from the live admission descriptors):**
- No `CALLCODE`/delegatecall, no `SELFDESTRUCT` âœ… (executor's `address(this).call` is a plain call, and the
...
- Every enabled callback implemented + `onlyPoolManager`-gated, PoolManager must be exactly
  `0x8366a39CC670B4001A1121B8F6A443A643e40951` âœ… (`src/ArbitHook.sol` â€” already the case)
- No proxy/upgrade, no mint/tax/pause surfaces on token âœ… (`src/ArbitToken.sol`)
- Return-delta flags stay false âœ… â€” note: this also means our fee-split accounting is internal only;
...
**Deploy-script lesson (anvil dry-run Sep 6):** `new ArbitHook{salt:}` inside a forge script is routed through the canonical keyless Create2Deployer `0x4e59b44847b379578588920cA78FbF26c0B4956C` â€” mine the salt against THAT address, not the script/sender, or `validateHookPermissions` reverts. Local dry-run path: `script/SetupAnvil.s.sol` (mock ARBT + PoolManager) â†’ `script/DeployArbit.s.sol` with `--unlocked --sender <anvil-acct>` (anvil accounts are unlocked; `--private-key` is not needed locally â€” and NEVER put a real key on a command line).

**Pool creation requirement:** the pool MUST be created with `fee = LPFeeLibrary.DYNAMIC_FEE_FLAG (0x800000)`. Static-fee pools silently ignore the `beforeSwap` fee return (`Hooks.sol:263`) â€” the demo will look live while charging the wrong fee. Fund the hook with ARBT at deploy so rewards / victim payouts / burns are backed by a real balance.

**Guardrail sources:** Uniswap v4 Security Framework (docs/protocols/v4/security), Trail of Bits "Building secure Uniswap v4 hooks" (Jul 2026 â€” 7 failure patterns), Uniswap AI `v4-security-foundations` skill (router-allowlist, gas budgets, absolute prohibitions), Beosin/Zealynx hook reviews (pool-binding, delta-conservation â‰  correctness, dust/`clear()`, nested-callback fuzzing).
...
```bash
#!/bin/sh
# .git/hooks/pre-commit â€” blocks key names/tokens/standalone 64-hex keys,
...
| 2 | `ArbitHook.sol` â€” beforeSwap fee override + afterSwap fee splitting + Buyback Guard |
| 3 | Full integration tests â€” all 3 participant flows + buyback scenarios + Slither + Sepolia deploy |
| 4 | Programmable CLI â†’ Robinhood Chain mainnet + Blockscout verify + website + X posts + submission form |
...
**Hero:** "Every trade makes Arbit stronger." Live stats above fold: total buybacks executed, total fees collected, current buyback pool balance, next buyback eligible time. The live data IS the hero â€” not an illustration.

**How it works:** Three participant lanes side by side â€” Human (protected, 20% to buyback), Agent (rewarded for good behavior), Bot (taxed, funds the system). Visual flow showing where assets go after each swap type.

**Register as agent:** Connect wallet â†’ stake Arbit â†’ start earning reduced fees + token rewards. Direct call to `ArbitRegistry.register()`.

**Design:** Near-black background (`#070B0E`), electric amber accent (`#F59E0B`) â€” amber signals value and yield, different from the typical DeFi green or blue. Space Grotesk for headings, JetBrains Mono for all live onchain data. No gradient cards. Data-dense, terminal-clean.
...
- [ ] Slither clean â€” no high/medium findings
- [ ] Deployed + verified on Robinhood Chain Testnet (46630) first â€” all 3 flows + Guard scenarios green on testnet
- [ ] All 3 participant flows demonstrated on testnet
- [ ] Buyback Guard tested â€” cooldown + minimum threshold verified
- [ ] Flash loan test passing â€” Guard holds
- [ ] Hook live on Programmable.market â†’ Robinhood Chain (chain ID 4663)
...
- [ ] X account active â€” 3+ posts before submission
...
- [ ] Pre-commit guard active â€” no keys in repo
- [ ] `.env.example` committed â€” real `.env` never committed
